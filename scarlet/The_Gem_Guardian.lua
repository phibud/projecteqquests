...
function event_timer(e)
	eq.stop_timer("delay");
	eq.set_proximity(e.self:GetX()-25, e.self:GetX()+25, e.self:GetY()-25, e.self:GetY()+25, -999999, 999999, true);
...
function event_proximity_say(e)
	...
	if (e.message:findi("gem")) then
		if (qglobals["wiz_epic"] == "1") then
			...
			eq.zone_emote(MT.Yellow, "Unfortunately my friend, you are not a student of the sorcerous arts, and cannot be trusted with information so vital to our cause.");
		end
	end
end
