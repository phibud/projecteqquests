...
function Efficiency_Say(e)
	if e.message:findi("hail") and not event_active then
		e.self:Say("'This is the Mastery of Efficiency trial. You must demonstrate your ability to function under less than perfect conditions, battling with limited resources. Are you ready to [begin]?");
...
function Efficiency_Timer(e)
	...
	elseif e.timer == "waves" then
		...
		if not el:IsMobSpawnedByNpcTypeID(307001) and not el:IsMobSpawnedByNpcTypeID(307002) and not el:IsMobSpawnedByNpcTypeID(307003) then 
